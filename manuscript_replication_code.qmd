---
title: "The Push and Pull of Exemptions and Incentives"
subtitle: "Supporting Content for UAR Submission"
format: 
  html:
    code-fold: true
    toc: true
...
filepath <- "../Merriman RA/ptax/"
nicknames <- readxl::read_excel(paste0(filepath, "Necessary_Files/muni_shortnames.xlsx"))

cook_sums <- read_csv(paste0(filepath, "Output/ptaxsim_cook_level_2006to2023.csv") )

muni_sums <- read_csv(paste0(filepath, "Output/ptaxsim_muni_level_2006to2023.csv") )

mc_burden <- read_csv(paste0(filepath, "Output/website_files/muni_mc_burden_", params$year, ".csv"))

muni_mc_sums <- read_csv(paste0(filepath, "Output/ptaxsim_muni_MC_2006to2023.csv") ) |>
  rename_all(~str_replace(., "muni_mc_", ""))


muni_cl_sums <- read_csv(paste0(filepath, "Output/ptaxsim_muni_class_summaries_2006to2023.csv") ) |>
  left_join(nicknames)

muni_shp <- read_sf(paste0(filepath, "Necessary_Files/muni_shp.gpkg"))
...
pin_data <- read_csv(paste0(filepath, "Output/Dont_Upload/0_joined_PIN_data_", params$year, ".csv"))


eq_factor <- read_csv(paste0(filepath, "Necessary_Files/eq_factor.csv")) |>
...
proj_xwalk <- readxl::read_xlsx(paste0(filepath, "Output/projects_checked_MAINFILE.xlsx")) |>
  rename(incent_prop2 = incent_prop)|>
...
class_dict <- read_csv(paste0(filepath, "Necessary_Files/class_dict_expanded.csv") ) |>
...
cook_MC_sums <- read_csv(paste0(filepath, "Output/ptaxsim_cook_MC_level_2006to2023.csv") )
...
```{r}
#| label: fig-FIGURE3-dotplotburdenshift-withouttext
#| fig-cap: "**Figure 3 for manuscript**"
...
Figure3 <- Figure3_data |>
 ...
 plot.background = element_rect(fill='transparent', color=NA),
  ...
  scale_color_brewer(palette="Paired", labels = c("Burden if No Exemptions", "Current Burden" ), direction = 1) +
...
ggsave(plot = Figure3, filename= "./paper-figures/Figure3.eps", dpi = "print")
ggsave(plot = Figure3, filename= "./paper-figures/Figure3.png", dpi = "print")

```

```{r}
#| label: fig-figure4-shareofcomindFMV
#| fig-cap: "**Figure 4 for Manuscript** Share of Commercial and Industrial FMV that has an incentive property class."
...
#ggsave(plot = figure4, filename= "./paper-figures/Figure4.eps", dpi = "print")
ggsave(plot = figure4, filename= "./paper-figures/Figure4.jpg", dpi = "print")
ggsave(plot = figure4, filename= "./paper-figures/Figure4.tif", dpi = "print")
ggsave(plot = figure4, filename= "./paper-figures/Figure4.png", dpi = "print")
...
```{r}
muni_rate <- read_csv(paste0(filepath, "Output/ptaxsim_muni_level_2006to2023.csv") ) |>
  filter(year == params$year) |>
  left_join(nicknames) |> 
...
muni_ratechange <- read_csv(paste0(filepath, "Output/website_files/muni_ratechange_", params$year, ".csv")) |> 
  filter(!clean_name %in% cross_county_line)
```
...
#| tbl-cap: "**Table 1 in Manuscript: Increase in Composite Property Tax Rate Due to Exemptions, 2023** from hypothetical scenario of taxing property that is current exempt due to exemptions."
...
#| tbl-cap: "**TABLE 3 for Manuscript: Composite Tax Rate Change** from hypothetical scenario of taxing incentive property at 25% of their FMV instead of 10% of their FMV."
...
#| fig-cap: "**Figure 5 Image for Academic Paper.** Only shows municipalities that had more than 1/2 percentage point change in tax rate. Municipalities with less than X amount are in grayscale."
#| fig-height: 10
#| fig-width: 6
...
figure5 <- figure9_data  |>
  ...
  theme( 
    ...
    axis.text.y = element_markdown( family = "Times New Roman",
        size = 16), # enable HTML styling in axis labels
    axis.text.y.left = element_markdown(    family = "Times New Roman",
        size = 16),
    # enable HTML styling in axis labels
...
ggsave(plot = figure5, filename= "./paper-figures/Figure5.eps", dpi = "print")
ggsave(plot = figure5, filename= "./paper-figures/Figure5.png", dpi = "print")

```

```{r}
#| label: fig-exemptions-overtime
#| fig-cap: "**FIGURE 1 in Manuscript: Residential Exempt EAV in Cook County**"
...
ggsave(plot = figure1, filename= "./paper-figures/Figure1.eps", dpi = "print")
ggsave(plot = figure1, filename= "./paper-figures/Figure1.png", dpi = "print")

```

```{r}
#| label: fig-exemptionsums-ResidentialEAV
#| fig-cap: "**FIGURE 2 in Manuscript: Exempt EAV as a Share of Residential Assessed Value by Municipality.** Share of residential EAV that is tax exempt due to homestead exemptions. Denominator is a municipality's residential EAV."
...
figure2 <- median_exempt |>
        left_join(nicknames) |> # to get name that joins with the muni_shp shapefiles
...
ggsave(plot = figure2, filename= "./paper-figures/Figure2.eps", dpi = "print")
ggsave(plot = figure2, filename= "./paper-figures/Figure2.png", dpi = "print")
...
#| tbl-cap: "**Similar to Table 2 in Manuscript**  Incentive FMV by Year, 2006 to 2023"

comm_ind_pins <- read_csv(paste0(filepath, "Output/comm_ind_PINs_2006to2023_timeseries.csv"))
...
#| tbl-cap: "Also similar to Table 2 in Manuscript."
...
```{r}
rate_change <- read_csv(paste0("../Merriman RA/ptax/Output/website_files/muni_ratechange_", params$year, ".csv"))


rate_change  <- rate_change |> 
  mutate(new_TEAV_no_tif_or_incent =  muni_eav_taxed + eav_tif_increment + forgone_EAV_incent,
         rate_no_tif_or_incent = round(final_tax_to_dist / new_TEAV_no_tif_or_incent, digits = 4),
        change_no_tif_or_incents = round(current_rate_avg - rate_no_tif_or_incent, digits = 4) )

rate_change |> 
  select(clean_name,  current_rate_avg, rate_no_tif_or_incent, change_no_tif_or_incents, pincount, pins_withincents, eav_tif_increment, eav_incents_inTIFs, new_TEAV_no_tif_or_incent) |> 
  DT::datatable()

```

```{r}
#| label: fig-JoshRachelPaper-dotplot3dots-focusonbigchanges-notiforincents
#| fig-cap: "**Additional comparison of incentive and TIF hypothetical impacts on tax rates** Only shows municipalities that had more than 1/2 percentage point change in tax rate. Municipalities with less than X amount are in grayscale."
#| fig-height: 10
#| fig-width: 6


# make dot graph
comparison_figure_data <- muni_ratechange |>  
  mutate(new_TEAV_no_tif_or_incent =  muni_eav_taxed + eav_tif_increment + forgone_EAV_incent,
         rate_no_tif_or_incent = round(final_tax_to_dist / new_TEAV_no_tif_or_incent, digits = 4),
        change_no_tif_or_incent = round(rate_current - rate_no_tif_or_incent, digits = 4) ) |>
  
  mutate(lowchange = ifelse(change_noInc < 0.015 & change_no_tif_or_incent < 0.015, TRUE, FALSE)
         ) |>
    filter(change_noInc > .015 | clean_name == "Chicago" | clean_name == "Cook" | change_no_tif_or_incent > 0.015) |>

 # filter(change_noInc > .005 | clean_name == "Chicago" | clean_name == "Cook" | change_noExe > 0.01) |>

  select(clean_name, rate_current, current_rate_avg, rate_noInc, lowchange, rate_no_tif_or_incent) |> 
#   rbind(addcook) |>
  ...
  pivot_longer(c(
                 "rate_noInc", 
                "current_rate_avg",
                "rate_no_tif_or_incent"), 
               names_to = "type", values_to = "tax_rate") |> 
  inner_join(order)  |>
  mutate(
    dotcolor = case_when(
     
      type == "rate_noInc" ~ "#ffffcc", 
      type == "rate_no_tif_or_incent" ~ "#6dae4f",
       type == "current_rate_avg" ~ "#253494",
    ),
    dotalpha = case_when(
      lowchange == TRUE ~ .8,
      T ~ 1 ),
...
label_vec <- setNames(comparison_figure_data$dotlabel, comparison_figure_data$clean_name)


comparison_figure <- comparison_figure_data  |>
  ggplot(aes(x = tax_rate, y= reorder(clean_name, rate_current)))+
  geom_line(aes(group = clean_name, alpha = dotalpha), show.legend = c(alpha = FALSE)
            )+ 
  geom_point(aes(fill=dotcolor, alpha = dotalpha ), color = "black", size=3, pch = 21, 
  ...
  theme( 
    ...
    plot.title.position = "plot",
    plot.background = element_rect(fill='transparent', color=NA), #transparent plot bg
    axis.text.y = element_markdown(size = 14), # enable HTML styling in axis labels
    axis.text.y.left = element_markdown(size = 14) # enable HTML styling in axis labels
  ) +
  scale_fill_identity(labels = rev(
    c(
    "No Incentive Classes",
         "No Incentives or TIFs",
...
tablelist <- list(
  ...
  "Table1_full" = table1JD_full, # increase in comp rate due to exemptions
   ...
   "Table2" =    Table2_allyears,     # FMV of incentive PINs. 
...
file_name = paste0("Manuscript_replication", "_", today_date, ".xlsx")
